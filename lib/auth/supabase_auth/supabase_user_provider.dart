...
class PMinumSupabaseUser extends BaseAuthUser {
  PMinumSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pMinumSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PMinumSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
